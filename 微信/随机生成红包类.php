...
{/*{{{*/
    ...
    {/*{{{*/
        ...
        {
            return array('error'=>'不能生成红包');
        }

        $data = array();
        if(false == is_int($this->num) || $this->num <= 0)
        {
            return array('error'=>'红包数量有问题');
        }
        $totalData = 0;
        ...
        if($totalData>$this->totalMoney){
            return array('error'=>'红包之和超过总额');
        }
        return $data;
    }/*}}}*/
...
$max = 5;
//生成配置参数的对象
$dto  = OptionDTO::create($amount,$num,$min,$max,'RandTriangle');
